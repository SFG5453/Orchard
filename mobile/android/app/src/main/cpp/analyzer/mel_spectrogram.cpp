...
// spreads quiet detail across the network's useful input range, so it is part
// of the model contract rather than a taste setting.
constexpr double kLogMultiplier = 1000.0;
constexpr double kAmplitudeFloor = 1e-10;

// Unnormalized in-place radix-2 FFT. kBeatSpectrogramFft is a power of two, so
// no generic padding is performed.
...
// Slaney mel scale: linear below 1 kHz, logarithmic above. This is the variant
// torchaudio uses by default, and it is not interchangeable with the HTK
// formula -- picking the wrong one silently shifts every filter.
...
// One triangular mel filter, stored as the run of FFT bins it actually covers.
//
// Sparse rather than a dense [bins][mels] matrix on purpose: a triangle spans a
// handful of bins, so the dense form is 98% zeros and costs 513 x 128 multiplies
// per frame -- around 800 million for a four-minute track, several seconds of
// pure zero-multiplying. Storing the run makes the same work about 1,000
...
// trained on that. Slaney-normalizing here would scale every band by its own
// width and quietly change the input distribution.
...
// Periodic Hann, matching torch.hann_window(periodic=True): the divisor is the
// window length, not length - 1. The symmetric variant used elsewhere in this
// addon would be a different window and a different spectrum.
...
BeatSpectrogram ComputeBeatSpectrogram(
  ...
  const size_t bins = kBeatSpectrogramFft / 2 + 1;
  // torchaudio's `normalized=True` divides the transform by the square root of
  // the window length.
