...
/** FP32 Beat This! through LiteRT's GPU compiled model, owned by the analysis thread. */
internal class Fp32BeatRunner(context: Context) : AutoCloseable {
    ...
    init {
        try {
            ...
            model = CompiledModel.create(
                context.assets,
                "beat_this_fp32.tflite",
                CompiledModel.Options(Accelerator.GPU, Accelerator.CPU).apply {
                    // DEFAULT precision produced constant logits on the Motorola razr 2023.
