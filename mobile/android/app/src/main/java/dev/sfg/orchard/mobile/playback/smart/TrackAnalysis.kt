 ...
 *
 * This is the contract between whatever produces analysis and the transition policy. Nothing here
 * is PCM: the policy and planner read these fields and their confidences, and never touch audio.
 * Every field is optional, because the ladder in [assessTransitionTier] is built to degrade on
 * missing evidence rather than to require it; an all-defaults instance is a legitimate input that
 * simply lands on the bottom rung.
 */
data class TrackAnalysis(
    /**
     * Blank means "no status was reported", which counts as ready. Any other value must be
     * [STATUS_READY] for the planner to trust the rest of the fields.
     */
    val status: String = "",
    ...
    val bpm: Double = 0.0,
    /**
     * Seconds per beat. Redundant with [bpm], but the analyzer measures it directly and it survives
     * tempo drift better, so it is preferred wherever both are available.
     */
    val beatInterval: Double = 0.0,
    /**
     * How far the beat grid can be trusted, 0..1. A catalog tempo lookup merges in at 0, so a
     * metadata BPM alone can never authorize beat-matching.
     */
    ...
    /**
     * Per-sample vocal activity, indexed against [energyCurve] sample times. Empty, or any length
     * other than the energy curve's, means "no evidence", which never blocks a transition.
     */
...
/**
 * A candidate point for a transition to enter or leave on. [score] is the analyzer's own
 * confidence; [type] is what it recognized, and carries its own weight during ranking.
 */
...
data class RankedMixCandidate(
    ...
    /**
     * Seconds of audible music this candidate would skip by ending the transition before the
     * content does. Mix-out only; always 0 for mix-in.
     */
...
/**
 * The degradation ladder. Ambition falls in explicit steps as certainty does, rather than letting
 * one engine quietly do beat math on junk data.
 */
