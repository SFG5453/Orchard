 ...
 *
 * The design follows the two-stage architecture every shipped auto-mix system converges on:
 * analysis happens ahead of playback, and the runtime only decides how ambitious a transition the
 * stored evidence can support. Ambition degrades in explicit tiers as certainty falls (see
 * [TransitionTier]) rather than letting one engine quietly do beat math on junk data.
 *
 * Every judgement here is made from stored analysis fields and their confidences; nothing in this
 * file touches PCM.
 */
...
/**
 * Below this on both tracks, even the DJ-assisted crossfade (beat-quantized anchors, EQ handoff) is
 * off the table and the mix degrades to a plain fade.
 */
...
/**
 * A vocal-activity mask value at or above this counts as singing. A fallback analyzer that emits a
 * flat 0.5 mask never trips vocal logic; only a real mask can.
 */
const val VOCAL_ACTIVE_THRESHOLD = 0.6

/**
 * How much of the outgoing track's remaining *music* a transition may skip by ending before its
 * content does. A transition is allowed to leave a short tail unplayed; it is not allowed to cut
 * the song short.
 *
 * This is the difference between mixing an outro and skipping one. The analyzer marks an outro up
 * to 48 seconds before content end, and the silence-cliff detector will call any gap past the
 * halfway mark a mix-out; anchoring to either without a budget throws away a minute of music the
 * listener can hear is still coming. The outro is the part of a track written to have something
 * else played over it, so the overlap belongs inside it and the transition still ends where the
 * content does.
 */
const val MAX_DISCARDED_MUSIC_SECONDS = 12.0

/**
 * Fraction of a track's own loud-end reference below which a sample counts as silence rather than
 * music, so a genuine gap costs nothing against the budget.
 */
private const val AUDIBLE_ENERGY_FRACTION = 0.1

/**
 * How much each candidate type is trusted as an entry point before scoring. Drops are where an
 * arrangement arrives, so they dominate; a pickup is just "the file starts making sound" and a
 * phrase boundary is only a grid line.
 */
...
/**
 * Mirrors the analyzer's own scoring of mix-out candidates, used when an analysis carries the
 * scalar fields but not the candidate list.
 */
...
/**
 * Halves or doubles [incomingBpm] until it is as close as possible to [outgoingBpm], the way a DJ
 * counts a 63 BPM track against a 126 BPM one.
 */
...
/**
 * Mean vocal activity over [start]..[end] on a track's own timeline, or null when the analysis
 * carries no usable mask there. The mask is indexed against [TrackAnalysis.energyCurve] times.
 */
...
/**
 * Both windows measurably singing at once. Null means "no evidence", which never blocks; absence
 * of a mask is not absence of a vocal, but acting on it would punish every track a fallback
 * analyzer handled.
 */
...
/**
 * Seconds of audible music in [start]..[end] on a track's own timeline, judged against the track's
 * own loud-end reference so the measure is independent of how the analyzer scales energy. Returns
 * null when there is no usable curve.
 */
...
/**
 * Ranks a track's analyzed mix-in candidates as entry points for a transition, best first.
 *
 * Selection is a scoring problem, not a type lookup: the analyzer's own score, the candidate type,
 * downbeat alignment, whether there is any intro before the point to bed under the outgoing track,
 * and how vocal that intro is all move a candidate up or down.
 */
...
private fun mixOutCandidatesOf(analysis: TrackAnalysis, contentEnd: Double): List<MixCandidate> {
    ...
    if (supplied.isEmpty()) {
        ...
        if (mixOut > 0 && mixOut < contentEnd - 1) {
            candidates += MixCandidate(mixOut, 0.95, "interior_mix_out")
        }
...
/**
 * Ranks a track's analyzed mix-out candidates as places for a transition to end, best first.
 *
 * Candidates that would skip more than [MAX_DISCARDED_MUSIC_SECONDS] of remaining music are dropped
 * outright: how confidently the analyzer marked a boundary is no argument for cutting a song short,
 * and both an outro marker and a mid-track silence gap will happily do exactly that. Silence is
 * free, so a genuine interior gap still wins the anchor it deserves.
 */
...
/**
 * Where the outgoing track's transition ends: the best-ranked mix-out candidate that stays inside
 * the discarded-music budget, or the end of content when none does.
 */
 ...
 *
 * Reasons are ordered most-disqualifying first so callers can surface `reasons.first()` as the
 * routing verdict.
 */
