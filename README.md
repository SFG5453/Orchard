...
Release builds include a pinned, mix-native
[All-In-One ONNX model](docs/smart-crossfade-models.md) for functional
structure, beats, and downbeats. A playback setting controls AI inference,
while the native analyzer and deterministic transition planner remain the
runtime fallback. Model-confirmed clean handoffs can use a quiet preroll into a
one-bar, four-beat takeover with a quantized bass swap.
