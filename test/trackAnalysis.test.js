...
test('accepts only current canonical analysis in the persisted cache contract', () => {
  const legacy = finalizeTrackAnalysis(rawAnalysis({ analysisVersion: 12 }));
  const current = finalizeTrackAnalysis(rawAnalysis({ analysisVersion: 13 }));

  assert.equal(isValidLocalAnalysis(legacy), false);
  assert.equal(isValidLocalAnalysis(current), true);
  assert.equal(AUDIO_ANALYSIS_VERSION, 13);
});
