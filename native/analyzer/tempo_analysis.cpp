...
// How much audio the onset envelope covers. The envelope now feeds beat
// *tracking* as well as tempo estimation, so it has to reach the end of the
// track: a transition's mix-out anchor sits in the outro, and a grid that stops
// three minutes short of it is extrapolation, not measurement. The cap is a
// memory guard for pathological input, not a tuning constant -- twenty minutes
// at 86 frames per second is under a megabyte of envelope.
constexpr double kMaxEnvelopeSeconds = 1200.0;

// How much of the envelope the autocorrelation tempo search reads. Bounded
// separately because Correlation() is O(n) per lag across ~110 lags, so letting
// it see a whole track would cost seconds; three minutes is ample to establish
// which metrical level a track is on, and the phase-locked grid below handles
// everything local from there.
...
// steady-state energy, then peak-normalize and sqrt-expand what remains so the
// quieter onsets still participate in correlation and phase scoring.
